...
- `switch-to-campus.ps1`：禁用 `LuYouQi`，启用 `XiaoYuanWang`，检查默认网关和当前启用物理网卡，并明确警告不要打开 VPN。
- `check-network.ps1`：不修改任何网卡，只检查 `XiaoYuanWang`、`LuYouQi`、默认路由和默认网关，弹窗提示当前是否适合打开 VPN。
- `install-shortcuts.ps1`：在桌面创建三个快捷方式，并尽量自动设置管理员运行和快捷键。
...
2. 打开“属性”。
3. 点击“快捷键”输入框。
4. 分别输入 `Ctrl + Alt + 1` / `Ctrl + Alt + 2` / `Ctrl + Alt + 3`。
5. 点击“应用”。
