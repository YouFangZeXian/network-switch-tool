...
- `switch-to-campus.ps1`：切换前先检测疑似 VPN / 代理 / 虚拟隧道是否仍在运行；如果检测到风险，会询问是否自动退出相关进程并停用相关后台服务。复检无风险后才会禁用 `LuYouQi`、启用 `XiaoYuanWang`，并明确警告不要打开 VPN。
...
切回校园网时，脚本会先做一轮检测。如果发现常见 VPN/代理进程、运行中的 VPN 服务，或 `Up` 状态的 TAP/TUN/Wintun/WireGuard/Tailscale/ZeroTier 等虚拟网卡，会先弹窗询问是否自动处理。

如果选择“是”：

1. 自动结束检测到的相关进程。
2. 停止并禁用检测到的相关后台服务。
3. 把被脚本禁用的服务记录到 `.state/disabled-vpn-services.json`。
4. 再次检测，确认无风险后才切入校园网。

如果自动处理后仍检测到风险，脚本会继续禁止切换校园网。

切回路由器时，脚本会读取 `.state/disabled-vpn-services.json`，恢复之前由脚本禁用的服务启动类型；如果服务原本处于运行状态，也会尝试启动回来。
...
- 发现风险时先询问是否自动退出/停用；复检仍有风险就直接阻止切换。
- 切回路由器后恢复由脚本禁用的 VPN/代理后台服务。
